...
for f in happ_crypto.py handlers.py marzban_api.py sub_proxy.py services.py tasks.py; do
...
async def main():
    async with httpx.AsyncClient(timeout=20) as client:
        ...
        for r in await conn.fetch('select telegram_id, marzban_username, subscription_url from marzvpn_users'):
            ...
            assert cleaned, r['marzban_username']
            for c in cleaned:
                assert 'security=none' in c and 'encryption=none' in c
                assert 'pbk=' not in c and 'sid=' not in c and 'flow=' not in c
            ...
            await conn.execute(
                ...
                sub, add, 'happ_add_plain_vless', datetime.now(timezone.utc), r['telegram_id'])
...
print(d)
assert "security=none" in d and "encryption=none" in d
assert "pbk=" not in d and "reality" not in d.lower()
print("SUB_OK")'
echo DONE
