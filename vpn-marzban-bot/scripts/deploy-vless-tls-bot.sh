...
async def main():
    async with httpx.AsyncClient(timeout=20) as client:
        ...
        for r in await conn.fetch('select telegram_id, marzban_username, subscription_url from marzvpn_users'):
            ...
            assert cleaned
            for c in cleaned:
                assert 'security=tls' in c and 'sni=svoygarage.ru' in c
                assert 'encryption=none' in c and 'pbk=' not in c
            ...
            await conn.execute(
                ...
                sub, add, 'happ_add_vless_tls', datetime.now(timezone.utc), r['telegram_id'])
...
print(d)
assert "security=tls" in d and "svoygarage.ru" in d
print("SUB_OK")'
echo DONE
