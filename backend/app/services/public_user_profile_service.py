...
def iter_public_profile_urls(db: Session, site_origin: str) -> list[tuple[str, str]]:
    ...
    for (code,) in sellers:
        if code and is_valid_public_code(code):
            items.append((f"{origin}/seller/{code}", "0.55"))
    ...
    for (code,) in buyers:
        if code and is_valid_public_code(code):
            loc = f"{origin}/buyer/{code}"
            if not any(existing[0] == loc for existing in items):
                items.append((loc, "0.5"))
