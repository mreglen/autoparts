...
class GarageUsedOrder(Base):
    __tablename__ = "garage_used_orders"
...
class GarageUsedOrderItem(Base):
    __tablename__ = "garage_used_order_items"

    id = Column(Integer, primary_key=True, index=True)
    order_id = Column(Integer, ForeignKey("garage_used_orders.id", ondelete="CASCADE"), index=True, nullable=False)
