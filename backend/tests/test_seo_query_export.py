...
_LETTERS_AND_SPACES = re.compile(r"^[^\W\d_]+(?: [^\W\d_]+)*$", re.UNICODE)


class NormalizeTests(unittest.TestCase):
    def test_normalize_and_clean_letters_only(self):
        self.assertEqual(normalize_query_key("  Купить   Фару  "), "купить фару")
        self.assertEqual(clean_query_display("б/у фара #12!"), "б у фара")
        self.assertEqual(clean_query_display("Bosch 1 987 302"), "Bosch")
        self.assertEqual(clean_query_display("Ростов-на-Дону"), "Ростов на Дону")


class GenerateProductQueriesTests(unittest.TestCase):
    def test_templates_include_name_brand_article_letters(self):
        seed = ProductSeed(name="Фара левая", brand="Bosch", article="ABC-123")
        queries = generate_queries_for_product(seed, start_index=0)
        joined = "\n".join(queries)
        self.assertTrue(any("купить Фара левая в" in q for q in queries))
        self.assertIn("Bosch ABC купить", joined)
        self.assertTrue(any(PRIMARY_CITY in q for q in queries))
        for q in queries:
            self.assertRegex(q, _LETTERS_AND_SPACES)
    ...
    def test_generate_from_products_respects_need_and_exclude(self):
        seeds = [
            ProductSeed(name=f"Деталь {chr(ord('а') + i)}", brand="ACME", article=f"ART{chr(ord('A') + i)}")
            for i in range(40)
        ]
        out = generate_from_products(seeds, need=200)
    ...
    def test_ekaterinburg_share_in_generated_geo_queries(self):
        seeds = [
            ProductSeed(name=f"Запчасть {chr(ord('а') + i)}", brand="Brand", article=f"ART{chr(ord('A') + i)}")
            for i in range(30)
        ]
        out = generate_from_products(seeds, need=200)
...
class BuildListTests(unittest.TestCase):
    def test_cap_and_real_first(self):
        ...
        seeds = [ProductSeed(name="Капот", brand="VW", article="ABC")]
        ...
        with patch(
            ...
            result = build_seo_queries_list(MagicMock(), limit=200)
