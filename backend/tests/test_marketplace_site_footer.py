...
class MarketplaceSiteFooterTests(unittest.TestCase):
    def test_disabled_returns_original(self):
        product = SimpleNamespace(id=16, brand="MANN", article="IF1009", internal_code="SG-16")
        self.assertEqual(
            append_marketplace_site_info("База", enabled=False, product=product),
            "База",
        )

    def test_appends_footer_with_product_url_and_internal_code(self):
        product = SimpleNamespace(
            id=16,
            brand="MANN",
            article="IF1009",
            internal_code="SG-0016",
        )
        with patch(
        ...
        self.assertIn("Внутренний код товара на сайте Свой Гараж: SG-0016", result)
        self.assertIn(FOOTER_MARKER, result.casefold())

    def test_does_not_duplicate_footer(self):
        product = SimpleNamespace(id=1, brand="A", article="B", internal_code="X1")
        footer = build_marketplace_site_footer(
            product_url="https://svoygarage.ru/part/1-A-B",
            internal_code="X1",
        )
        once = f"Текст\n\n{footer}"
        with patch(
    ...
    def test_footer_has_no_emoji(self):
        text = build_marketplace_site_footer(
            product_url="https://svoygarage.ru/part/1",
            internal_code="ABC",
        )
