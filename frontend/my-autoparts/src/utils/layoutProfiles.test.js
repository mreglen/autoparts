...
describe('chat layout classes', () => {
  ...
  it('uses the same mobile horizontal padding on all cabinet pages', () => {
    for (const pathname of ['/my-parts', '/autoservice/orders', '/autoservice/orders/new', '/sales/orders']) {
      const classes = getCabinetMainClasses({
        isChatsPage: false,
        pathname,
      });
      expect(classes).toContain('max-lg:px-3');
      expect(classes).not.toContain('max-lg:px-0');
      expect(classes).not.toContain('max-lg:px-1.5');
      expect(classes).not.toContain('max-lg:px-4');
    }
  });
});
