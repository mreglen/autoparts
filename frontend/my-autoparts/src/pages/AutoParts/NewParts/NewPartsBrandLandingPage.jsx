...
export default function NewPartsBrandLandingPage() {
  ...
  const loadPage = useCallback(async () => {
    ...
    setError(null);
    try {
      const resolved = await apiRequest(`/public/seo/landings/brand_new/${encodeURIComponent(brandSlug)}`);
      setLanding(resolved);
      ...
      const cards = await apiRequest(
        `/public/new-parts/cards?brand=${encodeURIComponent(brandName)}&page=${page}&page_size=${PAGE_SIZE}`,
      );
      setCardsData(cards);
    } catch (e) {
  ...
  const breadcrumbItems = useMemo(
    () =>
      buildBreadcrumbsForPath(`/autoparts/new/brand/${brandSlug}`, {
        brandName: landing?.brand_name || landing?.title_ru,
      }),
    [brandSlug, landing],
  );
    ...
    return (
      <div className="relative min-h-[50vh] px-4 py-16">
        ...
        <div className="mx-auto max-w-lg rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-gray-500">{error || 'Посадочная для этого бренда недоступна.'}</p>
      ...
      <div className="mx-auto max-w-7xl px-4 pt-6 sm:px-6 lg:px-8">
        ...
        <header className="mb-8">
          ...
          <p className="mt-2 text-sm text-gray-600">
            {cardsData?.total
              ? `В каталоге ${cardsData.total} позиций ${landing.brand_name || landing.title_ru}`
        ...
        <SeoCrossLinksSection kind="brand_new" slug={brandSlug} />
        ...
        {cardsData?.items?.length ? (
          ...
          <div className="rounded-2xl border border-dashed border-gray-300 bg-white/80 p-8 text-center text-sm text-gray-500">
            Пока нет карточек для этого бренда.{' '}
