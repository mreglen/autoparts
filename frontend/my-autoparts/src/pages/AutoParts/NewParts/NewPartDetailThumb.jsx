...
export default function NewPartDetailThumb({ imageUrl, attribution, alt, className = '' }) {
  ...
  return (
    <div className={`shrink-0 ${className}`}>
      <div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-xl bg-surface-subtle sm:h-24 sm:w-24">
        {src ? (
          <img
            src={src}
            alt={alt || ''}
      ...
      {src && attribution ? (
        <p className="mt-1 max-w-[6rem] truncate text-[10px] leading-tight text-ink-faint sm:max-w-[6.5rem]" title={attribution}>
          {attribution}
        </p>
      ) : null}
    </div>
