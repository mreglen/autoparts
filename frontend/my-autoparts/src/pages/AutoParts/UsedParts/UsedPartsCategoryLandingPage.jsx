...
export default function UsedPartsCategoryLandingPage() {
  ...
  const loadPage = useCallback(async () => {
    ...
    try {
      const resolved = await apiRequest(
        `/public/seo/landings/category_used/${encodeURIComponent(categorySlug)}`,
      );
      setLanding(resolved);
      const partTypeId = resolved?.part_type_id;
      if (!partTypeId) {
  ...
  const seo = useMemo(() => {
    ...
    return buildUsedPartsCategorySeo({
      landing,
  ...
  const breadcrumbItems = useMemo(
    () =>
      buildBreadcrumbsForPath(`/autoparts/used/category/${categorySlug}`, {
  ...
  return (
    <div className="relative pb-16">
      ...
      <div className="mx-auto max-w-7xl px-4 pt-6 sm:px-6 lg:px-8">
        ...
        <SeoCrossLinksSection kind="category_used" slug={categorySlug} />
