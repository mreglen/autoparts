...
export function buildUsedPartsCategorySeo({ landing, total = 0, items = [] }) {
  const category = landing?.title_ru || 'категория';
  const slug = landing?.slug || '';
  const canonicalPath = landing?.canonical_path || `/autoparts/used/category/${slug}`;
  const canonicalUrl = absoluteUrl(canonicalPath);

  const title =
    landing?.meta_title || `Б/у ${category} — купить от продавцов | Свой Гараж`;
  const description =
    landing?.meta_description ||
    (total > 0
      ? `Каталог б/у ${category.toLowerCase()}: ${total} объявлений, цены, доставка.`
      : `Каталог б/у ${category.toLowerCase()}: объявления, цены, доставка.`);
