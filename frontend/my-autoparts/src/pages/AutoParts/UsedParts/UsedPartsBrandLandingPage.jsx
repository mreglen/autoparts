...
export default function UsedPartsBrandLandingPage() {
  ...
  const loadPage = useCallback(async () => {
    ...
    try {
      const resolved = await apiRequest(
        `/public/seo/landings/brand_used/${encodeURIComponent(brandSlug)}`,
      );
      setLanding(resolved);
  ...
  const breadcrumbItems = useMemo(
    () =>
      buildBreadcrumbsForPath(`/autoparts/used/brand/${brandSlug}`, {
      ...
      <div className="mx-auto max-w-7xl px-4 pt-6 sm:px-6 lg:px-8">
        ...
        <header className="mb-8">
          ...
          <p className="mt-2 text-sm text-gray-600">
            {productsData?.total
              ? `${productsData.total} объявлений от продавцов`
        ...
        <SeoCrossLinksSection kind="brand_used" slug={brandSlug} />
        ...
        {productsData?.items?.length ? (
          ...
          <div className="rounded-2xl border border-dashed border-gray-300 bg-white/80 p-8 text-center text-sm text-gray-500">
            Пока нет объявлений для этого бренда.{' '}
