...
const DeliveryMethodsSection = ({ orgId }) => {
  ...
  const { allDeliveryMethods, orgDeliveryMethods, loadingDeliveryMethods, deliveryMethodsError } =
    useSelector((state) => state.organization);

  const loading =
    loadingDeliveryMethods.deliveryMethods || loadingDeliveryMethods.deliveryMethodAssignments;
  ...
  return (
    <Card>
      <h3 className="text-sm font-semibold text-ink">Способы доставки</h3>
      <p className="mb-4 mt-0.5 text-sm text-ink-muted">Что увидит покупатель при оформлении</p>
      ...
      {loading ? (
        ...
        <p className="py-4 text-sm text-ink-muted">Нет доступных способов доставки</p>
      )}
    </Card>
  );
};
