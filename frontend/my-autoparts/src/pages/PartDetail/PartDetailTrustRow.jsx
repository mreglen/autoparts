...
const TRUST_ITEMS = [
  {
    ...
    icon: (
      <svg className="h-3.5 w-3.5 text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
  ...
  return (
    <div className="mt-3 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
      {TRUST_ITEMS.map((item) => (
        <span
          key={item.label}
          className="inline-flex items-center gap-1.5 rounded-sg border border-brand-100 bg-brand-50 px-2.5 py-1.5 text-xs font-medium text-ink-soft"
        >
          {item.icon}
          {item.label}
        </span>
