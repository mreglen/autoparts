...
export default function CartPage() {
  ...
  const MobileCartItemCard = ({ item, seller }) => {
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
        <div className="flex justify-between items-start mb-3">
          <div className="flex-1 pr-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-gray-500 font-mono">{item.number}</span>
            </div>
          ...
          <div className="text-right flex-shrink-0">
            ...
            <div className="text-xs text-gray-600">{formatPrice(item.price)}/шт</div>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3 mb-3">
          <div>
            <div className="text-xs text-gray-500 mb-1">Дата поставки</div>
            <div className="text-sm text-gray-900">
              {item.deliveryDate ? formatDeliveryTime(item.deliveryDate) : 'Не указана'}
            </div>
          </div>
          <div>
            <div className="text-xs text-gray-500 mb-1">Кол-во</div>
            <div className="flex items-center justify-between">
              ...
              {item.quantity >= maxAllowed && maxAllowed < 10 && (
        ...
        <div className="flex justify-between items-center pt-3 border-t border-gray-100">
          ...
          >
            <img src="/img/trash_full.svg" alt="" className="w-4 h-4 mr-1 filter brightness-0 saturate-100 invert-16 sepia-84 saturate-7456 hue-rotate-0deg brightness-97 contrast-105" />
            Удалить
          </button>
    ...
    <div className="mt-5">
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          ...
          {/* Мобильная версия - карточки для всех продавцов */}
          <div className="md:hidden space-y-6">
            {Object.entries(groupedItems).map(([seller, items]) => (
              <div key={seller} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                {/* Заголовок продавца для мобильной версии */}
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="flex items-center space-x-1">
                        <input
                          ...
                          className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                        />
                        <label htmlFor={`deliverParts-mobile-${seller}`} className="text-xs font-medium text-gray-700">
                          Частями
                        </label>
                      </div>
                      <p className="text-xs text-gray-600">
                        {items.length} • {formatPrice(calculateSellerTotal(items))}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3 p-4">
                  {items.map((item) => (
                    <MobileCartItemCard key={item.id} item={item} seller={seller} />
                  ))}
                </div>
                  ...
                  <div className="flex flex-col space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {items.some(item => selectedItems.has(item.id)) && (
                        <>
                          <button
                            ...
                            className="flex-1 px-3 py-2 border border-red-300 text-xs font-medium rounded text-red-700 bg-white hover:bg-red-50 focus:outline-none focus:ring-1 focus:ring-red-500 min-w-[100px]"
                          >
                            Удалить выбранное
                          </button>
                          <button
                      ...
                      <button
                        ...
                        className="flex-1 px-3 py-2 border border-red-300 text-xs font-medium rounded text-red-700 bg-white hover:bg-red-50 focus:outline-none focus:ring-1 focus:ring-red-500 min-w-[80px]"
                      >
                        Удалить все
                      </button>
                      <button
