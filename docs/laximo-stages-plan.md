...
**Ключи:** только через `/admin-settings` (как OpenRouter). Продуктовая логика Laximo **не работает**, пока админ не сохранил credentials, не прошёл успешный тест API и не включил тумблер (и пока не исчерпана дневная квота).  
