...
Все действия выполняются в `/admin-settings` → блок **«Яндекс Товары / Вебмастер»**.

1. Сохраните OAuth `Client ID` / `Client secret` или вставьте готовый `access token` (кнопка «Сохранить токен»).
2. Подключите OAuth через **«Подключить Яндекс (OAuth code)»** или ручной токен.
3. Нажмите **«Включить интеграцию»** — система:
   - получит `user_id` через `GET /v4/user`;
   - добавит/найдет сайт в Вебмастере;
   - проверит `verification_state`;
   - запустит async-загрузку фида.
4. Для диагностики используйте **«Проверить состояние API»**.
5. Если API вернул необходимость ручной проверки:
   - перейти в документацию подтверждения прав:
   - <https://yandex.ru/dev/webmaster/doc/ru/concepts/verification>
6. После подтверждения прав повторить **«Включить интеграцию»** или **«Auto-try добавить/проверить host»**.
